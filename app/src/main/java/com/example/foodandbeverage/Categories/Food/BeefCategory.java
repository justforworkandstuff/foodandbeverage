...
public class BeefCategory extends AppCompatActivity {

    RecyclerView beefView;
    List<RecipeData> beefRecipeList;
    ProgressDialog progressDialog;
    EditText search_Feature;
    RecipeAdapter beefAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //initialization
        beefRecipeList = new ArrayList<>();
        search_Feature = findViewById(R.id.beef_searchFeature);
        beefView = findViewById(R.id.beef_recyclerView);

        //assigning view with layout manager
        GridLayoutManager gridLayoutManager = new GridLayoutManager(BeefCategory.this, 1);
        beefView.setLayoutManager(gridLayoutManager);

        progressDialog = new ProgressDialog(this);
        progressDialog.setMessage("Loading recipes..");

        beefAdapter = new RecipeAdapter(BeefCategory.this, beefRecipeList);
        beefView.setAdapter(beefAdapter);

        //connecting to database named "Beef"-
        databaseReference = FirebaseDatabase.getInstance().getReference("Beef");

        progressDialog.show();
        eventListener = databaseReference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                beefRecipeList.clear();

                for(DataSnapshot itemSnapshot: dataSnapshot.getChildren())
                {
                    RecipeData recipeData = itemSnapshot.getValue(RecipeData.class);
                    recipeData.setKey(itemSnapshot.getKey());
                    beefRecipeList.add(recipeData);
                }
    ...
    private void filter(String text)
    {
        ...
        for(RecipeData item: beefRecipeList)
        {
        ...
        beefAdapter.categoryFilteredList(filterList);
    }

    public void beef_upload(View view)
    {
